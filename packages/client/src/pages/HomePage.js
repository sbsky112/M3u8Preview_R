...
export function HomePage() {
    ...
    return (_jsxs("div", { className: "space-y-8", children: [continueWatching && continueWatching.length > 0 && (_jsx(ScrollRow, { title: "\u7EE7\u7EED\u89C2\u770B", children: continueWatching.map((item) => (item.media && (_jsx("div", { className: "w-[280px] sm:w-[320px] flex-shrink-0 snap-start", children: _jsx(MediaCard, { media: item.media, variant: "landscape", showProgress: true, progress: item.percentage, progressText: `${formatDuration(item.progress)} / ${formatDuration(item.duration)}` }) }, item.id)))) })), randomMedia && randomMedia.length > 0 && (_jsx(ScrollRow, { title: "\u968F\u673A\u63A8\u8350", moreLink: "/library", children: randomMedia.map((media) => {
                    const prog = progressMap?.[media.id];
                    return (_jsx("div", { className: "w-[140px] sm:w-[160px] lg:w-[170px] flex-shrink-0 snap-start", children: _jsx(MediaCard, { media: media, variant: "portrait", showProgress: !!prog, progress: prog?.percentage, completed: prog?.completed }) }, media.id));
                }) })), recentMedia && recentMedia.length > 0 && (_jsx(ScrollRow, { title: "\u6700\u8FD1\u6DFB\u52A0", moreLink: "/library", children: recentMedia.map((media) => {
                    const prog = progressMap?.[media.id];
                    return (_jsx("div", { className: "w-[140px] sm:w-[160px] lg:w-[170px] flex-shrink-0 snap-start", children: _jsx(MediaCard, { media: media, variant: "portrait", showProgress: !!prog, progress: prog?.percentage, completed: prog?.completed }) }, media.id));
                }) }))] }));
}
